...
class Pool {
  public:
    Pool(uint32_t num_threads)  {
        ...
        for (uint32_t i = 0; i < num_threads; ++i) {
            threads.emplace_back(&Pool::run, this);
        }
    }
    ...
    int64_t add_task(const Func &task_func, Args &&...args) {
        ...
        q.emplace(std::async(std::launch::deferred, task_func, args...),
                  task_idx);
        q_cv.notify_one();
        return task_idx;
    }
  ...
  private:
    ...
    std::queue<std::pair<std::future<void>, int64_t>>
        q; // очередь задач - хранит функцию(задачу), которую нужно исполнить и
           // номер данной задачи
    std::mutex q_mtx;
    std::condition_variable q_cv;

    std::unordered_set<int64_t>
        completed_task_ids; // помещаем в данный контейнер исполненные задачи
